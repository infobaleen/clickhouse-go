...
func TestParseEnum(t *testing.T) {
	...
	if err := ParseEnum([]byte(s), func(str []byte, num int) {
		resNumbers[fmt.Sprintf("%d", num)] = struct{}{}
	}); err != nil {
		panic(err)
	}

	if len(resNumbers) != len(refNumbers) {
		t.Fatal("length mismatch", len(resNumbers), len(refNumbers))
	}
